var phoneOperatorTable;
$(function() {
	initTable();
	$("#phoneOperatorAdd").click(function() {
		addPhoneOperator();
	});
	$("#phoneOperatorSearch").click(function() {
		searchPhoneOperator();
	});
});
//添加运营商
function addPhoneOperator() {
	layer.open({
		type : 2,
		area : [ '800px', '500px' ],
		fix : false, // 不固定
		...
		shade : 0.4,
		title : '运营商添加',
		content : ctx + '/redirect?page=phoneOperator/phoneOperatorAdd',
		end : function() {
			phoneOperatorTable.api().ajax.reload();
		}
	});
}
//跳转到修改页面
